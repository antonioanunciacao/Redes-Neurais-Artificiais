...
pesos_Perceptron <- model_Perceptron[[1]]

resultV <- Validation(X1, X2, k, eta, tol, maxepocas, par)

coeficientes <- -pesos_Perceptron[1:2]/pesos_Perceptron[3]
seqx = seq(0,6,6/100)
xi = as.matrix(seqx)
...
for(i in seqi){
  ...
  for(j in seqj){
    cj<-cj+1
    xy[ci,cj] <- sign(t(as.matrix(c(1,i,j))) %*% model_Perceptron[[1]])
  }
}
...
write.table(pesos_Perceptron/pesos_Perceptron[3], file = "pesos_perceptron.txt", sep = "\t",
