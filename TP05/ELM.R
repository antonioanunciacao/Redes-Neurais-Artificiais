...
s1 <- s2 <- 0.4
nc <- 100
X1<-cbind(matrix(rnorm(nc*2), ncol=2)*s1 + t(matrix((c(2,2)), ncol=nc, nrow=2)), -1)
X2<-cbind(matrix(rnorm(nc*2), ncol=2)*s2 + t(matrix((c(4,4)), ncol=nc, nrow=2)), 1)

plot(X1[,1],X1[,2], col = 'green', xlim = c(0,6), ylim = c(0,6), xlab = '', ylab = '')
par(new=T)
